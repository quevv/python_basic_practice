...
Bây giờ, hãy clone Git repository `intek-sm-python_basics.git` về `Devel` repository của bạn:
...
_Lưu ý: `{{username}}` tương ứng với username của bạn mà không có các ký tự `{{` và `}}`._
...
# Waypoint 4: Kiểm tra xem liệu có ít nhất 1 điều kiện (Conditions) là Đúng (True)
